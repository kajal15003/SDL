...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        plt.scatter(epoch, train_loss.avg)
...
def test(epoch):   
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            ...
            feat_pool, feat, feat_vis, feat_inf, out_u, feat_u, p_v, p_i = net(input, input, test_mode[0])
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(query_loader):
            batch_num = input.size(0)
            input = Variable(input.cuda())
            feat_pool, feat, feat_vis, feat_inf, out_u, feat_u, p_v, p_i = net(input, input, test_mode[1])
