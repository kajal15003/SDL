...
class ClassBlockS(nn.Module):
    def __init__(self, input_dim, 2, dropout=0.5, relu=True):
        super(ClassBlockS, self).__init__()
        classifier = []       
        if relu:
            classifier += [nn.LeakyReLU(0.1)]
        if dropout:
...
class embed_net(nn.Module):
    ...
    def forward(self, x1, x2, modal = 0 ):
        ...
        if self.training:
            #return self.l2norm(y3-y4), self.l2norm(y3-y5)
