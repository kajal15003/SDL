...
best_acc = 0 # best test accuracy start_epoch = 0
...
if dataset =='sysu':
    # testing set
    query_img, query_label, query_cam = process_query_sysu(data_path, mode = 
args.mode)
    gall_img, gall_label, gall_cam = process_gallery_sysu(data_path, mode = 
args.mode, trial = 0)
...
trainset.train_thermal_label)
    ...
    # testing set
    query_img, query_label = process_test_regdb(data_path, trial = args.trial, 
modal = 'visible')
    print(data_path)
    gall_img, gall_label = process_test_regdb(data_path, trial = args.trial, 
modal = 'thermal')
    
    gallset = TestData(gall_img, gall_label, transform = transform_test, 
img_size =(args.img_w,args.img_h))
    gall_loader = data.DataLoader(gallset, batch_size=args.test_batch, 
...
def extract_gall_feat(gall_loader):
    ...
    print('gallery',ngall,feature_dim,pool_dim)
    with torch.no_grad():
...
def extract_query_feat(query_loader):
    ...
    print('query',nquery,feature_dim,pool_dim)
    with torch.no_grad():
...
if dataset =='regdb':
    ...
    #plt.plot(query_feat,gallery_feat)
    #plt.show()
    # pool5 feature
    ...
    print('POOL5: top-1: {:.2%} | top-5: {:.2%} | top-10: {:.2%}| top-20: {:.2%}'.format(cmc_pool[0], cmc_pool[4], cmc_pool[9], cmc_pool[19]))
    print('mAP: {:.2%}'.format(mAP_pool))
    
elif dataset =='sysu':
    for trial in range(10):
        gall_img, gall_label, gall_cam = process_gallery_sysu(data_path, mode = 
args.mode, trial = trial)
        #plt.imshow(mpimg.imread(gall_img))
        #plt.show()
        trial_gallset = TestData(gall_img, gall_label, transform = 
transform_test,img_size =(args.img_w,args.img_h))
...
gall_cam)
        #plt.plot(query_feat,gall_feat)
        #plt.show()
        # pool5 feature
        distmat_pool = np.matmul(query_feat_pool, np.transpose(gall_feat_pool))
        cmc_pool, mAP_pool = eval_sysu(-distmat_pool, query_label, 
